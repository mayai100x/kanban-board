...
export default function EditTaskModal({ task, onClose, onSaved }: Props) {
  ...
  const [priority, setPriority] = useState<'P0' | 'P1' | 'P2' | 'P3'>(task.priority);
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
      <div className="bg-raised border border-border rounded-xl w-full max-w-md mx-4 p-6 shadow-2xl">
        <h2 className="text-lg font-semibold mb-1">✏️ Edit Task</h2>
        <p className="text-xs text-muted mb-4 font-mono">{task.id}</p>
